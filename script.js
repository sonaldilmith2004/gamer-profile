...
async function updateVisitCounter() {
    ...
    try {
        // --- 2. ALWAYS FETCH AND INCREMENT (with cache buster) ---
        // Increment on every load to ensure the user sees the 'updated' count immediately.
        const response = await fetch(`${BASE_URL}/up?t=${Date.now()}`);
...
if (muteToggleBtn && bgVideo) {
    ...
    if (enterScreen && enterBtn) {
        enterBtn.addEventListener('click', () => {
            ...
            // Unmute the background video on user gesture.
            // The video is already autoplaying muted, so we just flip muted=false.
            // Browsers always allow unmuting inside a direct user interaction handler.
            const targetVolume = volumeSlider ? parseFloat(volumeSlider.value) : 0.5;
            bgVideo.muted = false;
            bgVideo.volume = targetVolume > 0 ? targetVolume : 0.5;

            // Ensure it's playing (it should already be, but just in case)
            bgVideo.play().catch(() => {
                // If play() is somehow blocked, the video was already muted
                // so there's nothing more to do — do NOT remute.
                console.warn("bgVideo.play() rejected after enter click.");
            });

            // Start audio visualizer after a short delay (non-blocking)
            setTimeout(() => {
...
function renderIconVisualizer(canvas, ctx, dataArray, bufferLength, currentVol) {
    ...
    for (let i = 0; i < totalBars; i++) {
        ...
        if (ratio < 0.5) {
            ...
            r = 239 + sub * (249 - 239); g = 68 + sub * (115 - 68); b = 68 + sub * (22 - 68);
        } else {
            const sub = (ratio - 0.5) * 2;
            // Orange (#f97316) -> Amber (#fbbf24)
            r = 249 + sub * (251 - 249); g = 115 + sub * (191 - 115); b = 22 + sub * (36 - 22);
        }
...
let lastSpawnTime = 0;
const colors = ['#ef4444', '#f97316', '#fbbf24', '#dc2626', '#f59e0b'];
...
if (fireCanvas) {
    ...
    class Fireflake {
        constructor() {
            ...
            const themeColors = ['#ef4444', '#f97316', '#fbbf24', '#dc2626', '#f59e0b'];
